def solution(park, routes): 
    ...
    ways = {'E':[0,1], 'W':[0,-1], 'N':[-1,0], 'S':[1,0]}
    now = [[idx,s.index('S')] for idx,s in enumerate(park) if 'S' in s][0]
    
    def possible(w, nx, ny):
        if 0<=nx<rows and 0<=ny<cols:
            if w == 'E' and 'X' in park[now[0]][now[1]+1:ny+1]: return False
            elif w == 'W' and 'X' in park[now[0]][ny:now[1]]: return False
            elif w == 'S':
                for row in park[now[0]+1:nx+1]:
                    if row[ny]=='X': return False
            elif w == 'N':
                for row in park[nx:now[0]]:
                    if row[ny]=='X': return False
    ...
    for route in routes:
        w, n = route.split(' ')
        nx, ny = now[0]+ways[w][0]*int(n), now[1]+ways[w][1]*int(n)
        if possible(w, nx, ny): now = [nx, ny]
